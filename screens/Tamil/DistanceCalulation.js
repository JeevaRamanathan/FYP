...
class DistanceCalulation {
  data = (srcLatLng, desLatLng, wayPointLatLng, fn) => {
    axios
      .post(
        `https://maps.googleapis.com/maps/api/directions/json?destination=${desLatLng}&origin=${srcLatLng}&waypoints=optimize:true|${wayPointLatLng}&key=AIzaSyCYUfRcy0BLJRO7fuhHsI0dYxJqi0_X_E8`,
      )
      .then((res) => {
        res = res.data;
  ...
  data1 = (srcLatLng, desLatLng, fn) => {
    ...
    axios
      .post(
        `https://maps.googleapis.com/maps/api/directions/json?destination=${desLatLng}&origin=${srcLatLng}&key=AIzaSyCYUfRcy0BLJRO7fuhHsI0dYxJqi0_X_E8`,
      )
      .then((res) => {
        res = res.data;
        console.log(res);
